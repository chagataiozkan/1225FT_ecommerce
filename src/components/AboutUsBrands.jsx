export default function AboutUsBrands() {
  return (
    <section className="bg-[#FAFAFA] px-6 py-16 lg:px-20 lg:py-36">
      <div className="mx-auto flex max-w-6xl lg:max-w-355 flex-col items-center gap-20 lg:gap-40 lg:flex-row lg:items-center lg:justify-center">
        <img
          src="/images/fa-brands_hooli.svg"
          className="h-10 w-auto opacity-75"
        />

        <img
          src="/images/fa-brands_lyft.svg"
          className="h-12 w-auto opacity-75"
        />

        <img
          src="/images/fa-brands_pied-piper-hat.svg"
          className="h-14 w-auto opacity-75"
        />

        <img
          src="/images/fa-brands_stripe.svg"
          className="h-10 w-auto opacity-75"
        />

        <img
          src="/images/fa-brands_aws.svg"
          className="h-12 w-auto opacity-75"
        />

        <img
          src="/images/fa-brands_reddit-alien.svg"
          className="h-14 w-auto opacity-75"
        />
      </div>
    </section>
  );
}
